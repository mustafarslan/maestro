...
// Its own MAESTRO_HOME, seeded here. The first version inherited whatever the caller had,
// which made the result depend on ambient state: it passed for me because my shell pointed
// at a home I had just initialised, and failed in the clean-checkout gate because the real
// `~/.maestro` has migrations but no active playbook — so `get_playbook` answered nothing
// and the new assertions failed. A verification script that needs the world arranged
// beforehand verifies the arrangement as much as the code.
const home = mkdtempSync(join(tmpdir(), "maestro-mcp-check-"));
const env = { ...process.env, MAESTRO_HOME: home };
try {
  const seeded = spawnSync(binary, ["init"], { env, encoding: "utf8" });
  if (seeded.status !== 0) {
    console.log(
      `\ncould not initialise a temporary MAESTRO_HOME: ${seeded.stderr?.slice(0, 200)}\n`,
    );
    process.exit(2);
  }

  const child = spawn(binary, ["mcp"], { stdio: ["pipe", "pipe", "pipe"], env });
  let out = "";
  let err = "";
